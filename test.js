...
function read() {
    ...
    for (var i = 0; i < mdList.sites.length; i++) {
        let data = fs.readFileSync("D:\\学习\\github.io\\mds\\" + mdList.sites[i].path, "utf-8")
        var a = data.indexOf("---");
        var b = data.lastIndexOf("---");
        data = `{"path": "${path.parse(mdList.sites[i].path, "utf8").name}",${data.slice(a + 6, b - 2)}`
        console.log(data)
...
for (i = 0; i < jsonFiles.length; i++) {
    ...
    // }
    var markdown = require("C:\\Users\\18483\\AppData\\Roaming\\npm\\node_modules\\markdown").markdown;
    ...
    const tail = `</div><script src="../js/app.js"></script></body></html>`
